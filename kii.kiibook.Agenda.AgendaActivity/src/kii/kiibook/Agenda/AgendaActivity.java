...
public class AgendaActivity extends FragmentActivity implements
		OnDateChangeListener, OnTouchListener {
	...
	private void updateList() {
		...
		Iterator<NewEvent> it = DataShared.getInstance().getListEvents()
				.iterator();

		ArrayList<NextEventsListView> events = new ArrayList<NextEventsListView>();
		while (it.hasNext()) {
			NewEvent ev = it.next();
			events.add(new NextEventsListView(getDateString(ev.getDate()), ev
					.getType(), ev.getWhat() + " - " + ev.getDescription()));
		}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			Toast.makeText(this, "Action UP: " + event.getAction(),
